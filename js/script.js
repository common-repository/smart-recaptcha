...
(function($) {
	$(document).ready( function() {
		 ...
		 * @since 4.2.3
		 */
		var tabs = $( '#re_cptch_settings_tabs_wrapper' );
		if ( tabs.length ) {
			var carousel        = $( '#re_cptch_settings_slick' ),
				tabs_panel      = $( '#re_cptch_settings_tabs' ),
				tab_index_field = $( 'input[name="re_cptch_active_tab"]' ),
			...
			function set_carousel_height( window ) {
				if( window.width() <= 480 ) {
					if ( ! carousel.hasClass( 'hu_cptch_slick_mobile' ) )
						carousel.addClass( 'hu_cptch_slick_mobile' )
				} else {
					carousel.removeClass( 'hu_cptch_slick_mobile' );
				}
			}

			/* jQuery tabs initialization */
			tabs.tabs({
				active: re_cptch_vars.start_tab,
				create: function( event, ui ) {
					set_tabs_height( $( '#re_cptch_settings_tabs .ui-tabs-active a' ).attr( 'href' ) );
				}
			...
			var anchor       = window.location.hash,
				start_slide  = re_cptch_vars.start_tab,
				current_slide;
			if ( anchor != "" && anchor.match( /\#cptch\_(.)*\_tab/ ) ) {
				current_slide = $( '#re_cptch_settings_slick div:has(a[href="' + anchor + '"])' );
			...
			var enable_option  = $( "input[name*='[enable]']" ),
				image_format   = $( '#hu_cptch_operand_format_images' ),
				image_options  = $( '.re_cptch_images_options' ),
				package_list   = $( ".re_cptch_tabs_package_list:not(.cptch_pro_pack_tab)" ),
			...
			enable_option.click( function() {
				...
				if ( next.find( "input[name*='[use_general]']" ).is( ':checked' ) ) {
					...
					var rows = current.closest( '.re_cptch_form_tab' ).find( 'tr, .hu_pro_version_bloc' ).not( '.hu_cptch_form_option_enable' );
			...
			function cptch_image_options() {
				...
				package_list.each( function() {
					notice = image_format.prev( '.hu_cptch_enable_images_notice' );
					if ( ! notice.length )
						return;
			...
			package_list.find( 'input' ).change( function() {
				var pack_wrapper = $( this ).closest( '.re_cptch_tabs_package_list' ),
					notice       = pack_wrapper.prev( '.hu_cptch_enable_images_notice' );
				if ( ! notice.length )
					return;
				if (
		 ...
		 */
		$( 'button[name="re_cptch_show_blacklist_form"]' ).click( function() {
			$( this ).parent( 'form' ).hide();
			$( '.re_cptch_blacklist_form' ).show();
			return false;
		});
		 ...
		 */
		$( 'button[name="re_cptch_show_whitelist_form"]' ).click( function() {
			$( this ).parent( 'form' ).hide();
			$( '.re_cptch_whitelist_form' ).show();
			return false;
		});

		var limit_options = $( '.hu_cptch_limt_options' );
		$( 'input[name="re_cptch_use_time_limit"]' ).each( function() {
			if ( ! $( this ).is( ':checked' ) )
				limit_options.hide();
		}).click( function() {
			if ( $( this ).is( ':checked' ) )
		...
		$( 'input[name="re_cptch_use_la_whitelist"]' ).click( function() {
			$( this ).closest( 'form' ).submit();
		});
		/*  add to whitelist my ip */
		$( 'input[name="re_cptch_add_to_whitelist_my_ip"]' ).change( function() {
			if ( $( this ).is( ':checked' ) ) {
				var my_ip = $( 'input[name="re_cptch_add_to_whitelist_my_ip_value"]' ).val();
				$( 'input[name="re_cptch_add_to_whitelist"]' ).val( my_ip ).attr( 'readonly', 'readonly' );
			} else {
				$( 'input[name="re_cptch_add_to_whitelist"]' ).val( '' ).removeAttr( 'readonly' );
			}
		});
